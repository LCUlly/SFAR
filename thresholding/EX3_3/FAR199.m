...
function [x_threshold_199,x_199,num_199,Err_199,Err_threshold_199] = FAR199(noise_level)
...
for i = 1:n1+1
    for j = 1:n2+1
        ...
        KK = diag([1/1,ones(1,m1-1),1/1])*KK*diag([1/1,ones(1,m2-1),1/1]);
        K(i+(j-1)*(n2+1),:) = reshape(KK,1,(m1+1)*(m2+1));
    end
end
A = tau1*tau2*K;% Rectangle integral formula
xx_real = ((t1'-1/2).^2+(t2-1/2).^2<=9/64).*(4*sin(pi*t1').*sin(pi*t2)); % good 0.2011
...
for n = 1:N
    ...
    if n==N || Residual(n)/norm(y_real - y)<1.01
        ...
        if num ==1
            x_199 = x;
            num_199 = n;
            x_threshold_199 = x.*(1-(abs(x)<1));
            Err_threshold_199 = norm(x_threshold_199-x_real,2)/norm(x_real,2);
            xx = reshape(x_threshold_199,m1+1,m2+1);
            [T1,T2] = meshgrid(t1',t2);
            ...
            % 创建 title
            title({'\theta = 1.99'});
            figure(3); subplot(2,1,1),plot(max(1,n-30):n+1,Err_199(max(1,n-30):n+1))
