...
for n = 1:N
    ...
    if n==N || Residual(n)/norm(y_real - y)<1.01
        ...
        if num ==1
            x_15 = x;
            num_15 = n;
            x_threshold_15 = x.*(1-(abs(x)<1));
            Err_threshold_15 = norm(x_threshold_15-x_real,2)/norm(x_real,2);
            xx = reshape(x_threshold_15,m1+1,m2+1);
            [T1,T2] = meshgrid(t1',t2);
            ...
            % 创建 title
            title({'\theta = 1.5'});
            figure(3); subplot(2,1,1),plot(max(1,n-30):n+1,Err_15(max(1,n-30):n+1))
    ...
    % 创建 title
    title({'\theta = 1.5'});
    figure(3); subplot(2,1,1),plot(max(1,n-100):n+1,Err_15(max(1,n-100):n+1))
